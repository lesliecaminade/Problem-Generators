...
class multipleFunctions2Class:
    ...
    def __init__(self):
        ...
        expression_to_substitute = random.randint(-2,2)*x + coefficients[12]
        ...
        if operations == 0:#simple addition
            soln = func1.subs(x,expression_to_substitute) + func2.subs(x,expression_to_substitute)
...
g(x) = {b:s}
Find f({c:s}) + g({c:s})
                """.format(a = str(func1),b = str(func2), c = str(expression_to_substitute))
            if temp == 1:
                problem = """f(x) = {a:s}
g(x) = {b:s}
Find (f+g)({c:s})
...
Find (f/g)({c:s})
        ...
        if operations == 3:#composition
            partial_solution = func2.subs(x,expression_to_substitute)
...
print('MULTIPLE FUNCTIONS OPERATIONS')
print()

for i in range (1,10):
    A = multipleFunctions2Class()
